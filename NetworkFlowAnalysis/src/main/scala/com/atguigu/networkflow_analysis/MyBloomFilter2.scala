	...
	def main(args: Array[String]): Unit = {
		...
		val dataStream: DataStream[String] = env.readTextFile("Data/UserBehavior.csv")
		  ...
		  val dataStreams=env.readTextFile("Data/UserBehavior.csv")
			...
			.process(new MyUvWithBloomFiter())
...
class MyTrigger() extends Trigger[UserBehavior, TimeWindow] {
	override def onElement(element: UserBehavior, timestamp: Long,
	                       window: TimeWindow, ctx: Trigger.TriggerContext): TriggerResult = TriggerResult.CONTINUE
...
class MyUvWithBloomFiter() extends ProcessAllWindowFunction[UserBehavior, UvCount, TimeWindow] {
